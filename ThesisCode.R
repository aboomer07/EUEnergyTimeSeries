...
#dirJacob <- Insert your path here
#dirMoon <- Insert your path here
#dirFarid <- Insert your path here
...
#tokenJacob <- insert token here
#tokenMoon <- insert token here
tokenCurr <- tokenAndy
...
finalClean <- function(df, outType) {
	...
	df$GenNull <- apply(dplyr::select(df, genCols), 1, function(x) all(is.na(x)))
	...
	for (year in 2015:2019) {#Create year dummies
		df[, paste0("year", year) := fifelse(yearNum == year, 1, 0)]}

	for (hour in 0:23) {#Create hour number dummies
		df[, paste0("hour", hour) := fifelse(df$hourNum == hour, 1, 0)]}

	for (month in 1:12) {#Create month number dummies
...
# print("=======================================================================")
# initModel <- runPanel(dfs[["Belgium"]], initForm, "within")
# print("=======================================================================")
...
# print("=======================================================================")
# dynModel <- runPanel(dfs[['Belgium']], dynform, "within")
# print("=======================================================================")
...
# regions <- c("Belgium", "Czechia", "DE-AT-LU", "Denmark_1", "France", 
# 	"Hungary", "Italy", "Netherlands", "Portugal", "Spain", "Sweden")

# regions <- unique(df$regionArea)
# regions <- regions[regions != "DE-AT-LU"]

regions <- c("Belgium", "Denmark_1", "France", "Hungary", "Netherlands", 
	"Poland", "Portugal", "Spain", "Sweden")

covars <- c("pct_Renew", "RenewError", "impExp", "LoadError", "Load", "GenError", 
	"pctNon_Coal", "pctNon_Gas", "pctNon_Hydro", "pctNon_Nuclear")

initPlots <- function(df, regions, lags, col) {
	test <- data.table(seq(1:(lags + 1)))
	names(test) <- c("Lag")
	for (reg in regions) {
		test[, paste0("", reg) := acf(select(getReg(df, reg), col), 
			lag.max = lags, plot = FALSE)$acf]
	}
	compare <- ggplot(melt(test[2:dim(test)[1], ],
		measure.vars = regions,
		variable.name = "Region", value.name = "Correlation"),
		aes(x = Lag, y = Correlation, color = Region)) + 
		geom_line()
	plot(compare)
}

createForm <- function(depVar, covars, depLags = c(), dummies = c()) {

	#Using reformulate so we can pass strings and lists to create formulas
	xVals <- covars
	if (length(depLags) > 0) {xVals <- c(xVals,
		paste0("shift(", depVar, ", ", depLags, ")"))}
	if (length(dummies) > 0) {xVals <- c(xVals, dummies)}
	form <- reformulate(response = depVar, termlabels = xVals)
...
getCycles <- function(df, regions, numHours) {

	resP <- periodogram(df[df$regionArea == "France", "Residuals"])

	fullP = data.table(Frequency = resP$freq, France = resP$spec)
	fullP$Hours <- 1 / fullP$Frequency

	hourPlot <- ggplot(data = fullP[fullP$Hours <= numHours, ], 
		aes(x = Hours, y = France)) +
		geom_line()

	pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/FranceResPGram.pdf")
	plot(hourPlot)
	dev.off()

	fullP <- fullP[fullP$Hours <= numHours, ]

	for (region in regions[regions != "France"]) {
		resP <- periodogram(df[df$regionArea == region, "Residuals"])
		resP <- data.table(Frequency = resP$freq, Power = resP$spec)
		names(resP) <- c("Frequency", region)
		resP$Hours <- 1 / resP$Frequency
		resP <- resP[resP$Hours <= numHours, ]
		fullP <- merge(fullP, resP, by = c("Frequency", "Hours"), all.x = TRUE)
	}

	top3 <- data.table(regionArea = regions)
	top3 <- top3[, c("First", "Second", "Third") := list(1, 1, 1), ]

	for (region in regions) {
		regVec <- fullP[order(fullP[[region]], decreasing = TRUE), ]$Hours
		top3 <- top3[regionArea == region, 
			c("First", "Second", "Third") := list(regVec[1], regVec[2], regVec[3]),]
	}

	fullPlot <- ggplot(data = melt(top3, id.vars = "regionArea", 
		variable.name = "CycleRank", value.name = "CycleFrequency")) +
		geom_bar(stat = 'identity', 
			aes(x = regionArea, y = CycleFrequency, fill = CycleRank)) +
		facet_grid(col = vars(CycleRank)) +
		geom_hline(yintercept = 24) +
		geom_hline(yintercept = 168) + 
		coord_flip()

	pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/ResPGram.pdf")
	plot(fullPlot)
	dev.off()
...
getACF <- function(df, regions, numLags) {

	resACF <- acf(df[df$regionArea == "France", "Residuals"], 
		lag.max = numLags, plot = FALSE)

	fullResACF <- data.frame(Lag = 1:length(resACF$acf), Residuals = resACF$acf)
	fullResACF$regionArea <- "France"

	for (region in regions[regions != "France"]) {
		resACF <- acf(df[df$regionArea == region, "Residuals"],
			lag.max = numLags, plot = FALSE)
		resACF <- data.frame(Lag = 1:length(resACF$acf), Residuals = resACF$acf)
		resACF$regionArea <- region
		fullResACF <- rbind(fullResACF, resACF)
	}

	initACF <- acf(df[df$regionArea == "France", "actVol"],
			lag.max = numLags, plot = FALSE)
	fullInitACF <- data.frame(Lag = 1:length(initACF$acf), actVol = initACF$acf)
	fullInitACF$regionArea <- "France"

	for (region in regions[regions != "France"]) {
		initACF <- acf(df[df$regionArea == region, "actVol"],
			lag.max = numLags, plot = FALSE)
		initACF <- data.frame(Lag = 1:length(initACF$acf), actVol = initACF$acf)
		initACF$regionArea <- region
		fullInitACF <- rbind(fullInitACF, initACF)
	}

	fullACF <- merge(fullResACF, fullInitACF, by = c("Lag", "regionArea"))

	franceACF <- ggplot(data = fullACF[fullACF$regionArea == "France", ], 
		aes(x = Lag, y = Residuals)) + geom_line()

	pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/FranceResACF.pdf")
	plot(franceACF)
	dev.off()

	allACFPlot <- ggplot(data = fullACF, 
		aes(x = Lag, y = Residuals, color = regionArea)) + geom_line()

	pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/FullResACF.pdf")
	plot(allACFPlot)
	dev.off()

	fullACF$Diff <- fullACF$actVol - fullACF$Residuals

	allDiffPlot <- ggplot(data = fullACF, 
		aes(x = Lag, y = Diff, color = regionArea)) + geom_line()

	pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/FullDiffACF.pdf")
	plot(allDiffPlot)
	dev.off()
	
}

getInit <- function(series, var, covars) {
	model <- lm(createForm(var, covars), data = series)
	return(model)
}

getDummy <- function(currRes) {
	covars <- c(paste0("hour", 0:22), paste0("month", 1:11), paste0("Day", 1:6))
	form <- createForm("Residuals", covars)
	model <- lm(form, data = currRes)
	return(model)
}

getARIMA <- function(currRes) {
	model <- arima(currRes$Residuals, order = c(1, 0, 1))
	return(model)
}

getSARIMA <- function(currRes) {
	model <- arima(currRes$Residuals, order = c(2, 0, 1), 
		seasonal = list(order = c(1, 1, 1), period = 24))
	model2 <- arima(model$residuals, order = c(0, 0, 0), seasonal = 
		list(order = c(1, 0, 1), period = 168))
	return(model2)
}

getPAR <- function(currRes) {
	currRes$sin24 <- sin(1 * 2 * pi * (currRes$hourNum + 1) / 24) +
		sin(2 * 2 * pi * (currRes$hourNum + 1) / 24) + 
		sin(3 * 2 * pi * (currRes$hourNum + 1) / 24) + 
		sin(4 * 2 * pi * (currRes$hourNum + 1) / 24)

	currRes$cos24 <- cos(1 * 2 * pi * (currRes$hourNum + 1) / 24) +
		cos(2 * 2 * pi * (currRes$hourNum + 1) / 24) + 
		cos(3 * 2 * pi * (currRes$hourNum + 1) / 24) + 
		cos(4 * 2 * pi * (currRes$hourNum + 1) / 24)

	currRes$sin168 <- sin(1 * 2 * pi * (currRes$hourNum + 1) / 168) +
		sin(2 * 2 * pi * (currRes$hourNum + 1) / 168) + 
		sin(3 * 2 * pi * (currRes$hourNum + 1) / 168) + 
		sin(4 * 2 * pi * (currRes$hourNum + 1) / 168)

	currRes$cos168 <- cos(1 * 2 * pi * (currRes$hourNum + 1) / 168) +
		cos(2 * 2 * pi * (currRes$hourNum + 1) / 168) + 
		cos(3 * 2 * pi * (currRes$hourNum + 1) / 168) + 
		cos(4 * 2 * pi * (currRes$hourNum + 1) / 168)


	currRes$sin24AR1 <- shift(currRes$Residuals, 1) * currRes$sin24
	currRes$cos24AR1 <- shift(currRes$Residuals, 1) * currRes$cos24
	currRes$sin168AR1 <- shift(currRes$Residuals, 1) * currRes$sin168
	currRes$cos168AR1 <- shift(currRes$Residuals, 1) * currRes$cos168

	currRes$sin24AR2 <- shift(currRes$Residuals, 2) * currRes$sin24
	currRes$cos24AR2 <- shift(currRes$Residuals, 2) * currRes$cos24
	currRes$sin168AR2 <- shift(currRes$Residuals, 2) * currRes$sin168
	currRes$cos168AR2 <- shift(currRes$Residuals, 2) * currRes$cos168

	currRes$sin24AR24 <- shift(currRes$Residuals, 24) * currRes$sin24
	currRes$cos24AR24 <- shift(currRes$Residuals, 24) * currRes$cos24
	currRes$sin168AR24 <- shift(currRes$Residuals, 24) * currRes$sin168
	currRes$cos168AR24 <- shift(currRes$Residuals, 24) * currRes$cos168

	currRes$sin24AR168 <- shift(currRes$Residuals, 168) * currRes$sin24
	currRes$cos24AR168 <- shift(currRes$Residuals, 168) * currRes$cos24
	currRes$sin168AR168 <- shift(currRes$Residuals, 168) * currRes$sin168
	currRes$cos168AR168 <- shift(currRes$Residuals, 168) * currRes$cos168

	model <- lm(Residuals ~ sin24AR1 + cos24AR1 + sin168AR1 + cos168AR1 +
		sin24AR2 + cos24AR2 + sin168AR2 + cos168AR2 +
		sin24AR24 + cos24AR24 + sin168AR24 + cos168AR24 +
		sin24AR168 + cos24AR168 + sin168AR168 + cos168AR168, data = currRes,
		na.action = na.exclude)
	return(model)
}

getSS <- function(currRes, nIter) {
	ss <- AddLocalLinearTrend(list(), currRes$Residuals)
	ss <- AddTrig(ss, currRes$Residuals, period = 24, frequencies = 1:4)
	ss <- AddTrig(ss, currRes$Residuals, period = 168, frequencies = 1:4)
	ss <- AddAr(ss, currRes$Residuals, lags = 2)
	model <- bsts(currRes$Residuals, state.specification = ss, 
		niter = nIter)
	return(model)
}

getResFrame <- function(df, region) {

	currRes <- df[df$regionArea == region, ]

	# currRes <- currRes[1:200, ]

	dumMod <- getDummy(currRes)
	print("Dummy done")
	arimaMod <- getARIMA(currRes)
	print("ARIMA done")
	sarimaMod <- getSARIMA(currRes)
	print("SARIMA done")
	parMod <- getPAR(currRes)
	print("PAR done")
	ssMod <- getSS(currRes, 100)
	print("SS Done")

	df1 <- data.table(1:dim(currRes)[1],
		dumMod$residuals, arimaMod$residuals, sarimaMod$residuals,
		parMod$residuals,
		residuals.bsts(ssMod, burn = SuggestBurn(0.1, ssMod), mean.only = TRUE))
	names(df1) <- c("Index", "Dummies Model", 
			"ARIMA Model", "SARIMA Model",
			"Periodic Model", "Structural Model")

	df1 <- melt(df1, id.vars = "Index",
		measure.vars = c("Dummies Model", "ARIMA Model", 
			"SARIMA Model", "Periodic Model", "Structural Model"),
		variable.name = "ModelType", value.name = "Residuals")
...
getBoxTest <- function(df, numLags) {
	for (i in 1:numLags) {
		df[, paste0("", i) := Box.test(Residuals, lag = i, fitdf = 1)$p.value, 
			by = list(ModelType, Region)]
	}

	df <- df[, -c("Index", "Residuals")]
	df <- data.table(melt(df, id.vars = c("ModelType", "Region"),
		variable.name = "LagValue", value.name = "pValue"))
	df <- df[!duplicated(df), ]
	df$LagValue <- as.numeric(df$LagValue)

	return(df)
}

getRMSE <- function(resFrame) {
	resFrame <- resFrame[, "RMSE" := sqrt(mean(Residuals**2)), 
		by = list(ModelType, Region)]
	resFrame <- resFrame[, -c("Index", "Residuals")]
	resFrame <- resFrame[!duplicated(resFrame), ]
...
for (region in regions) {
	df[df$regionArea == region, ]$Residuals <- getInit(df[df$regionArea == region, ], "actVol", covars)$residuals
}

df$weekDay <- wday(df$datetime)
df$date <- as_date(df$datetime)
for (day in 1:7) {
	df[, paste0("Day", day) := fifelse(df$weekDay == day, 1, 0)]}
df <- df[order(datetime), ]

getCycles(df, regions, 200)
getACF(df, regions, 200)

break

for (i in 1:length(regions)) {
	if (i == 1) {
		resFrame <- getResFrame(df, regions[i])
	}
	else {
		resFrame <- rbind(resFrame, getResFrame(df, regions[i]))
	}
}

serCorFrame <- getBoxTest(resFrame, 200)

serCorPlot <- ggplot(serCorFrame) +
	geom_line(aes(x = LagValue, y = pValue, color = ModelType)) +
	geom_hline(yintercept = 0.05) +
	facet_wrap(~ Region, ncol = 3)

pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/BoxTestPlot.pdf")
plot(serCorPlot)
dev.off()

rmseFrame <- getRMSE(resFrame)
rmsePlot <- ggplot(rmseFrame, aes(x = ModelType, y = RMSE)) +
	geom_bar(stat = "identity", aes(fill = ModelType)) +
	facet_wrap( ~ Region, ncol = 3) +
	coord_flip()

pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/RMSEPlot.pdf")
rmsePlot
dev.off()


testPlot <- ggplot(data = test, aes(x = RMSE_Ratio, y = pValue, color = Region)) + 
	geom_point()

pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/RMSE_BoxCompare.pdf")
testPlot
dev.off()

acfTest <- expand.grid(unique(resFrame$Region), 1:200, unique(resFrame$ModelType))
names(acfTest) <- c("Region", "Lag", "ModelType")
acfTest <- acfTest[order(c(acfTest$Region, acfTest$ModelType, acfTest$Index)), ]
acfTest$ACF <- 1
acfTest <- acfTest[!duplicated(acfTest), ]
acfTest <- na.omit(acfTest)

for (region in unique(resFrame$Region)) {
	for (model in unique(resFrame$ModelType)) {
		test1 <- acf(resFrame[(resFrame$Region == region)&(resFrame$ModelType == model), ]$Residuals,
				lag.max = 200, plot = FALSE)
		acfTest[(acfTest$Region == region)&(acfTest$ModelType == model), ]$ACF <- test1$acf

	}
}

acfTestPlot <- ggplot(data = acfTest, aes(x = Lag, y = ACF)) +
	geom_line() +
	facet_grid(row = vars(Region), col = vars(ModelType), scales = "free_y")
pdf("/Users/andrewboomer/Desktop/M1_Courses/Thesis/ACFTest.pdf")
plot(acfTestPlot)
dev.off()
